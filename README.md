...
- **Beeswarm plot** — global feature importance with direction and magnitude per student
- **Bar plot** — mean absolute feature importance ranking across all students
- **Waterfall plot** — why this specific student was predicted as Dropout
- **Force plot** — same as waterfall in a compact horizontal format
- **Dependence plot** — how one feature affects dropout risk across all students

### DiCE Counterfactuals

Using [DiCE](https://github.com/interpretml/DiCE) to generate actionable recommendations restricted to features a student or advisor can realistically change:
...
Example output for an educator:
> *"Student #553 is predicted to drop out (98.83% risk). If their 2nd semester approved units increased and tuition fees were brought up to date, this prediction would change to Graduate."*
...
```
├── dataset.csv                    # Raw original dataset
├── modified_datapipeline.ipynb    # EDA + full preprocessing pipeline
├── model_build.ipynb              # Model training, evaluation and risk report
├── XAI.ipynb                      # SHAP and DiCE explainability
├── train_data.csv                 # SMOTE-balanced scaled training set
├── train_data_raw.csv             # Unscaled training set
├── test_data.csv                  # Scaled holdout test set
├── test_data_raw.csv              # Unscaled holdout test set
├── enrolled_inference.csv         # Enrolled students for dropout prediction
├── student_risk_report.csv        # Final output — risk scores per student
├── lr_model.pkl                   # Saved Logistic Regression model
├── rf_model.pkl                   # Saved Random Forest model
├── xgb_model.pkl                  # Saved XGBoost model
├── stacking_model.pkl             # Saved Stacking Meta-Learner model
...
| `shap` | Global + local model explainability |
...
Open notebooks in the following order in Google Colab or Jupyter:

```
1. modified_datapipeline.ipynb   → preprocessing and feature engineering
2. model_build.ipynb             → model training and risk report generation
3. XAI.ipynb                     → SHAP explanations and DiCE counterfactuals
```
